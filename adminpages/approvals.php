...
?>
	...
	<ul class="subsubsub">
		<li class="all"><a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=pmpro-approvals&s=' . urlencode( $s ) . "&l=$l&status=all" ) ); ?>" class="
											<?php
											if ( $status == 'all' ) {
										?>
									 current<?php } ?>"><?php _e( 'All', 'pmpro-approvals' ); ?></a></li> |
		<li class="pending"><a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=pmpro-approvals&s=' . urlencode( $s ) . "&l=$l&status=pending" ) ); ?>" class="
												<?php
												if ( $status == 'pending' || empty( $status ) ) {
											?>
										 current<?php } ?>"><?php _e( 'Pending', 'pmpro-approvals' ); ?></a></li> |
		<li class="approved"><a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=pmpro-approvals&s=' . urlencode( $s ) . "&l=$l&status=approved" ) ); ?>" class="
													<?php
													if ( $status == 'approved' ) {
												?>
											 current<?php } ?>"><?php _e( 'Approved', 'pmpro-approvals' ); ?></a></li> |
		<li class="denied"><a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=pmpro-approvals&s=' . urlencode( $s ) . "&l=$l&status=denied" ) ); ?>" class="
												<?php
												if ( $status == 'denied' ) {
											?>
...
selected="selected"<?php } ?>><?php _e( 'All Levels', 'pmpro-approvals' ); ?></option>
	...
	if ( $theusers ) {
		...
		if ( $l == 1 ) {
?>
<?php echo intval( $totalrows ); ?> <?php _e( 'applications awaiting review', 'pmpro-approvals' ); ?>.<?php } ?>
		</p>
		<?php
	}
	?>
	<table class="widefat">
		<thead>
			<tr class="thead">
				...
				<th><a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=pmpro-approvals&s=' . urlencode( $s ) . '&limit=' . $limit . '&pn=' . $pn . '&sortby=user_registered' ) ); ?>
										<?php
...
class="alternate"<?php } ?>>
						...
						<td class="username column-username">
							...
							<?php if ( current_user_can( 'edit_users' ) ) { ?>
									<strong><a href="<?php echo esc_url_raw( 'user-edit.php?user_id=' .$theuser->ID ); ?>"><?php echo esc_html( $theuser->user_login ); ?></a></strong>
								<?php } else { ?>
									<strong><a href="<?php echo esc_url_raw( 'admin.php?page=pmpro-approvals&user_id=' . $theuser->ID ); ?>"><?php echo esc_html( $theuser->user_login ); ?></a></strong>
								<?php } ?>
							<br />
							<?php
							...
							<td>
								<?php
								echo esc_html( $auser->membership );
								?>
							</td>						
							<td>										
								<?php
								...
								if ( PMPro_Approvals::isApproved( $theuser->ID ) || PMPro_Approvals::isDenied( $theuser->ID ) ) {
									...
									if ( ! PMPro_Approvals::getEmailConfirmation( $theuser->ID ) ) {
										...
										//link to unapprove
										?>
										[<a href="javascript:askfirst('Are you sure you want to reset approval for <?php echo $theuser->user_login; ?>?', '?page=pmpro-approvals&s=<?php echo urlencode( $s ); ?>&l=<?php echo $l; ?>&limit=<?php echo intval( $limit ); ?>&status=<?php echo $status; ?>&sortby=<?php echo $sortby; ?>&sortorder=<?php echo $sortorder; ?>&pn=<?php echo intval( $pn ); ?>&unapprove=<?php echo $theuser->ID; ?>&pmpro_approvals_nonce=<?php echo urlencode( $pmpro_approvals_nonce ); ?>');">X</a>]
										<?php
									}
								} else {
									?>
																			
									<a href="?page=pmpro-approvals&s=<?php echo urlencode( $s ); ?>&l=<?php echo $l; ?>&limit=<?php echo intval( $limit ); ?>&status=<?php echo esc_html($status); ?>&sortby=<?php echo esc_html($sortby); ?>&sortorder=<?php echo esc_html($sortorder); ?>&pn=<?php echo intval( $pn ); ?>&approve=<?php echo $theuser->ID; ?>&pmpro_approvals_nonce=<?php echo urlencode( $pmpro_approvals_nonce ); ?>">Approve</a> |
									<a href="?page=pmpro-approvals&s=<?php echo esc_html( $s ); ?>&l=<?php echo $l; ?>&limit=<?php echo intval( $limit ); ?>&status=<?php echo $status; ?>&sortby=<?php echo esc_html( $sortby ); ?>&sortorder=<?php echo esc_html( $sortorder ); ?>&pn=<?php echo intval( $pn ); ?>&deny=<?php echo $theuser->ID; ?>&pmpro_approvals_nonce=<?php echo urlencode( $pmpro_approvals_nonce ); ?>">Deny</a>
									<?php
								}
								?>
							</td>
	...
	<?php
	echo pmpro_getPaginationString( $pn, $totalrows, $limit, 1, esc_url_raw( get_admin_url( null, '/admin.php?page=pmpro-approvals&s=' . urlencode( $s ) ) ), "&l=$l&limit=$limit&status=$status&sortby=$sortby&sortorder=$sortorder&pn=" );
	?>
